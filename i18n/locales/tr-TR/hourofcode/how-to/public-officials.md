...
1. Kod Zamanı'nı desteklemek bilgisayar bilimleri eğitiminin neden önemli olduğunu gösteren **çağrı cihazımızı** okuyun. [Buradan kılavuza ulaşabilirsiniz.](/files/hoc-one-pager-public-officials-2016.pdf)

2. **Yerel okul ve bölgeleri işe alın.** [Bu e-posta'yı](%= resolve_url('/promote/resources#sample-emails') %) veya [bu öneri yazısını](%= resolve_url('/promote/stats') %) bir başlangıç noktası olarak kullanın ve bunun [nasıl yapıldığını ](%= resolve_url('/how-to') %) okul ve bölgelere anlatın.

3. **Bir Kod Zamanı toplantısına ev sahipliği yapın.** [Nasıl davet hazırlanır kılavuzumuz](%= resolve_url('/how-to/events') %)'u inceleyerek, örnek gösteri çalşması, medyaya ulaşım kiti ve diğer davet planlama kaynaklarımızdan yararlanın. Bir davete ev sahipliği yapmak Kod Zamanı'nı destelemek ve katılımı arttırmak için harika bir fırsat.
